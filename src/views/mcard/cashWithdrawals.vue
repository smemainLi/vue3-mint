<template>
  <div class="cash-withdrawals">
    ...
    <amount-input :tipsTitle="inputTitle" :tipsContent="inputContent" @getAmountMoney="getAmountMoneyVal"></amount-input>
...
export default {
  ...
  methods: {
    ...
    confirmWithdrawals () {
      ...
      console.log('sdf,hk')
      this.$router.push({ path: '/order/payPassword', query: { withdrawAmount: this.withdrawAmount } })
    },
    /* 获取输入金额 */
    getAmountMoneyVal (amountMoney) {
      ...
      if (amountMoney && amountMoney !== '0') {
        this.inputTitle = '最小可提现金额：'
        this.inputContent = `￥${this.minAmount}`
        if (this.isBinding === 'hasCard' && Number(amountMoney) <= this.takeAbleBalance && Number(amountMoney) >= this.minAmount) this.isAgree = true
        else this.isAgree = false
      } else {
        this.inputTitle = '可提现金额：'
        this.inputContent = `￥${this.takeAbleBalance}`
      }
    }
  },
  created () {
